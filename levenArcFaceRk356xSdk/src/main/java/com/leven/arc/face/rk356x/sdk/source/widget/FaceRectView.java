package com.leven.arc.face.rk356x.sdk.source.widget;
...
public class FaceRectView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!drawInfoList.isEmpty()) {
            for (int i = 0; i < drawInfoList.size(); i++) {
                drawFaceRect(context, canvas, drawInfoList.get(i), DEFAULT_FACE_RECT_THICKNESS, paint);
            }
        }
    }
     ...
     * @param paint             画笔
     */
    private static void drawFaceRect(Context context, Canvas canvas, DrawInfo drawInfo, int faceRectThickness, Paint paint) {
...
//            }
            ...
            boolean isShowFaceInfo = ConfigUtil.getEnableFaceViewInfo(context);
            if (attributeInfo != null && isShowFaceInfo) {
                paint.setStyle(Paint.Style.FILL_AND_STROKE);
