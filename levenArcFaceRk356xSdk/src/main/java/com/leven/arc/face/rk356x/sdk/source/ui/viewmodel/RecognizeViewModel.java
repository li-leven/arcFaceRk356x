...
import com.leven.arc.face.rk356x.sdk.source.callback.InitCallback;
import com.leven.arc.face.rk356x.sdk.source.callback.RecognizeResultCallback;
...
import com.leven.arc.face.rk356x.sdk.source.ui.model.PreviewConfig;
import com.leven.arc.face.rk356x.sdk.source.util.ConfigUtil;
import com.leven.arc.face.rk356x.sdk.source.util.FaceRectTransformer;
import com.leven.arc.face.rk356x.sdk.source.util.face.FaceHelper;
import com.leven.arc.face.rk356x.sdk.source.util.face.RecognizeCallback;
import com.leven.arc.face.rk356x.sdk.source.util.face.constants.LivenessType;
import com.leven.arc.face.rk356x.sdk.source.util.face.constants.RecognizeColor;
import com.leven.arc.face.rk356x.sdk.source.util.face.constants.RequestFeatureStatus;
import com.leven.arc.face.rk356x.sdk.source.util.face.model.FacePreviewInfo;
import com.leven.arc.face.rk356x.sdk.source.util.face.model.RecognizeConfiguration;
import com.leven.arc.face.rk356x.sdk.source.widget.FaceRectView;
...
public class RecognizeViewModel extends ViewModel implements RecognizeCallback {
    ...
    //是否开启人脸识别
    private boolean faceListenerStatus = true;
    //同一人是否可以多次注册
    private boolean registerMultiple = true;
    //人脸识别结果
    private RecognizeResultCallback recognizeResultCallback;
    ...
    public void setRecognizeResultCallback(RecognizeResultCallback recognizeResultCallback) {
        this.recognizeResultCallback = recognizeResultCallback;
    }
     ...
     */
    private void registerFace(final byte[] nv21, FacePreviewInfo facePreviewInfo, String id, String name) {
        ...
        registerNv21Disposable = Observable.create((ObservableOnSubscribe<Boolean>) emitter -> {
                    ...
                    int res = registerEngine.init(context, DetectMode.ASF_DETECT_MODE_IMAGE, DetectFaceOrientPriority.ASF_OP_0_ONLY,
                            1, FaceEngine.ASF_FACE_RECOGNITION);
                    if (res == ErrorInfo.MOK) {
                        float similar = ConfigUtil.getRecognizeThreshold(context);
                        boolean success = FaceServer.getInstance().registerNv21(context, nv21.clone(), previewSize.width,
                                previewSize.height, facePreviewInfo, id, name, registerMultiple, similar, frEngine, registerEngine);
                        registerEngine.unInit();
                        emitter.onNext(success);
                    } else {
                        emitter.onError(new SdkException(res, "注册引擎初始化错误"));
//                        emitter.onNext(false);
                ...
                .subscribeWith(new DisposableObserver<Boolean>() {
                    ...
                    public void onNext(Boolean success) {
                        if (onRegisterFinishedCallback != null) {
                            FaceEntity faceEntity = facePreviewInfo.getFaceEntity();
                            onRegisterFinishedCallback.onRegisterFinished(facePreviewInfo, faceEntity, success, 0, "");
                        }
                    ...
                    public void onError(Throwable e) {
                        ...
                        if (onRegisterFinishedCallback != null) {
                            onRegisterFinishedCallback.onRegisterFinished(facePreviewInfo, null, false, code, message);
                        }
                        updateRegisterStatus(REGISTER_STATUS_DONE);
                    }
    ...
    public void init(Context context, InitCallback callback) {
        ...
        int ftCode = ftEngine.init(context, DetectMode.ASF_DETECT_MODE_VIDEO, ConfigUtil.getFtOrient(context),
                ConfigUtil.getRecognizeMaxDetectFaceNum(context), ftEngineMask);
        if (ftCode != ErrorInfo.MOK && callback != null) {
            callback.onInitResult(false, ftCode, "初始化失败");
        }
        ...
        int frCode = frEngine.init(context, DetectMode.ASF_DETECT_MODE_IMAGE, DetectFaceOrientPriority.ASF_OP_0_ONLY,
                10, frEngineMask);
        if (frCode != ErrorInfo.MOK && callback != null) {
            callback.onInitResult(false, frCode, "初始化失败");
        }
//        frInitCode.postValue(frEngine.init(context, DetectMode.ASF_DETECT_MODE_IMAGE, DetectFaceOrientPriority.ASF_OP_0_ONLY,
//                10, frEngineMask));
        FaceServer.getInstance().initFaceList(context, frEngine, (faceCount, status) -> loadFaceList = true, true);
        ...
        if (enableLive) {
            ...
            int flCode = flEngine.init(context, DetectMode.ASF_DETECT_MODE_IMAGE,
                    DetectFaceOrientPriority.ASF_OP_ALL_OUT, 10, flEngineMask);
            if (flCode != ErrorInfo.MOK && callback != null) {
                callback.onInitResult(false, flCode, "初始化失败");
            }
...
//                    DetectFaceOrientPriority.ASF_OP_ALL_OUT, 10, flEngineMask));
        ...
        Disposable disposable = Observable.just(true).observeOn(AndroidSchedulers.mainThread()).subscribe(aBoolean -> {
            if (similarPass) {
                ...
                if (recognizeResultCallback != null) {
                    recognizeResultCallback.onRecognizeResult(true, compareResult, live, 0, "");
                }
            }else{
                //人脸未注册
                if (recognizeResultCallback != null) {
                    recognizeResultCallback.onRecognizeResult(false, compareResult, live, ExceptionEnum.FACE_NOT_REGISTER, "人脸未注册");
                }
            }
        });
    }
     ...
     * @return 当前帧的检测结果信息
     */
    public List<FacePreviewInfo> onPreviewFrame(byte[] nv21, boolean doRecognize, String id, String name) {
        if (faceHelper != null) {
            if (!loadFaceList) {
                return null;
            }
            ...
            if (registerStatus == REGISTER_STATUS_READY && !facePreviewInfoList.isEmpty()) {
                ...
                if (facePreviewInfo.getMask() != MaskInfo.WORN) {
                    registerFace(nv21, facePreviewInfoList.get(0), id, name);
                } else {
                    if (onRegisterFinishedCallback != null) {
                        onRegisterFinishedCallback.onRegisterFinished(null, null, false, ExceptionEnum.REGISTER_MASK_ERROR, "注册要求不戴口罩");
                    }
//                    Toast.makeText(ArcFaceApplication.getApplication(), "注册照要求不戴口罩", Toast.LENGTH_SHORT).show();
    ...
    public void setFaceListenerStatus(boolean faceListenerStatus) {
        this.faceListenerStatus = faceListenerStatus;
    }

    public void setRegisterMultiple(boolean registerMultiple) {
        this.registerMultiple = registerMultiple;
    }

    //判断注册的id是否存在
    public boolean checkFaceById(String userId) {
        List<FaceEntity> faceList = FaceServer.getInstance().getFaceRegisterInfoList();
        if(faceList == null || faceList.isEmpty()){
            return false;
        }
        for (FaceEntity faceEntity: faceList) {
    ...
    //重新加载人脸
    public void initFaceList() throws SdkException {
        FaceServer.getInstance().reInit(new FaceServer.OnInitFinishedCallback() {
            @Override
            public void onFinished(int faceCount, boolean status) {
